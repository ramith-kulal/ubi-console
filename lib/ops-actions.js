...
export const OPS_TABLE_KEYS = ['APPLICANT', 'CUSTID_DETAILS', 'CLOGIN'];
...
export const SEARCH_FIELDS = {
  APPLICANT: ['custId', 'id', 'userId', 'branch_code', 'appStatus'],
...
export const ACTIONS = {
  'set-app-status': {
    ...
    build({ stateGroup, id, status }) {
      ...
      const { table } = resolveTable(stateGroup, 'APPLICANT');
      return {
        table,
  ...
  'clear-bhoomi-files': {
    label: 'Clear bhoomi land files',
    tableKey: 'APPLICANT',
    destructive: true,
    build({ stateGroup, id }) {
      const safeId = assertSafeValue(id, 'id');
      const { table } = resolveTable(stateGroup, 'APPLICANT');
      // Mirrors ubi-backend/src/user/user-exp-routes.js:295, which clears this
      // exact path when a land record has to be re-fetched.
      return {
        table,
        summary: 'Empty docs.assets.bhoomi.files on 1 applicant row',
        sql:
          `UPDATE ${table} a SET a.docs.assets.bhoomi.files=[] ` +
          `WHERE a.id=${quoteLiteral(safeId)}`,
      };
    },
  },

  'clear-bhoomi-land-files': {
    label: 'Clear bhoomi_land files',
    tableKey: 'APPLICANT',
    destructive: true,
    build({ stateGroup, id }) {
      const safeId = assertSafeValue(id, 'id');
      ...
      return {
        table,
        summary: 'Empty docs.assets.bhoomi_land.files on 1 applicant row',
        sql:
          `UPDATE ${table} a SET a.docs.assets.bhoomi_land.files=[] ` +
          `WHERE a.id=${quoteLiteral(safeId)}`,
      };
    },
  },
  ...
  'clear-satsure-input': {
    ...
    build({ stateGroup, id }) {
      ...
      return {
        table,
        summary: 'Empty docs.assets.satSure.input on 1 applicant row',
        sql:
          `UPDATE ${table} a SET a.docs.assets.satSure.input=[] ` +
          `WHERE a.id=${quoteLiteral(safeId)}`,
      };
    },
  },
