...
/**
 * The payload a query confirm token commits to: the normalized statement text.
 *
 * Binding the exact text is the point — a token minted after previewing
 * `DELETE ... WHERE id='1'` must not authorise `DELETE ... WHERE id='2'`, so the
 * preview a user approved always describes the statement that runs.
 */
export function queryConfirmPayload({ normalizedSql }) {
  return `query|${normalizedSql}`;
}

/**
 * The payload a bypass-flag change commits to.
 *
 * Binds BOTH digests: the file as it was when the operator reviewed it, and the
 * file that will be written. Same-file means nobody edited over SSH in between;
 * same-next-file means the bytes applied are the bytes shown in the diff.
 */
export function bypassConfirmPayload({ targetKey, currentSha256, nextSha256 }) {
  return `bypass|${targetKey}|${currentSha256}|${nextSha256}`;
}

/** The payload a bypass restore commits to: the target, the file now, the backup. */
export function bypassRestoreConfirmPayload({ targetKey, currentSha256, backupId }) {
  return `bypass-restore|${targetKey}|${currentSha256}|${backupId}`;
}

/** The payload a single-row (key-based) mutation token commits to. */
export function rowConfirmPayload({ action, table, key }) {
  // Key fields are sorted so an equivalent key object always yields one payload.
  const sortedKey = Object.keys(key || {})
    .sort()
    .map((k) => `${k}=${String(key[k])}`)
    .join(',');
  return `row|${action}|${table}|${sortedKey}`;
}
