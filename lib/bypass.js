...
function loadTargets() {
  ...
  for (const [key, t] of Object.entries(raw)) {
    ...
    for (const field of ['label', 'pm2Name']) {
    ...
    if (typeof t.configPath !== 'string' || !path.isAbsolute(t.configPath)) {
      throw new Error(`bypass-targets.json: ${key}.configPath must be an absolute path`);
    }
    // Optional: backups default to a hidden directory beside the file itself.
    if (t.backupsDir != null && !path.isAbsolute(t.backupsDir)) {
      throw new Error(`bypass-targets.json: ${key}.backupsDir must be absolute`);
    }
...
export function getBypassTarget(key) {
  ...
  return {
    key,
    ...target,
    // Backups live beside the file they are backups of.
    backupsDir:
      target.backupsDir || path.join(path.dirname(target.configPath), '.bypass-backups'),
  };
}
